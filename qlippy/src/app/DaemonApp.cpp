...
DaemonApp::DaemonApp(QObject *parent)
    : QObject(parent), m_storage(std::make_unique<Storage>()), m_ipcServer(std::make_unique<IpcServer>()), m_waylandConn(std::make_unique<WaylandConnection>())
{
}
...
void DaemonApp::stop()
{
    ...
    if (m_waylandConn)
    {
        m_waylandConn->disconnectFromDisplay();
    }
    if (m_ipcServer)
        m_ipcServer->stop();
    if (m_storage)
...
void DaemonApp::onCommandReceived(IpcMessage msg, QLocalSocket *conn)
{
    ...
    if (msg.cmd == IpcCmd::Ping)
        handlePing(conn);
    else if (msg.cmd == IpcCmd::ListItems)
        handleListItems(conn);
    else if (msg.cmd == IpcCmd::SearchItems)
        handleSearchItems(msg, conn);
    else if (msg.cmd == IpcCmd::CopyItem)
        handleCopyItem(msg, conn);
    else if (msg.cmd == IpcCmd::DeleteItem)
        handleDeleteItem(msg, conn);
    else if (msg.cmd == IpcCmd::PinItem)
        handlePinItem(msg, conn);
    else if (msg.cmd == IpcCmd::ClearHistory)
        handleClearHistory(conn);
    else if (msg.cmd == IpcCmd::GetStatus)
